...
public class AirWeatherTask extends AsyncTask<Void, Void, String> {
    ...
    String longitude;// = Double.parseDouble(MainActivity.longti); // 경도 127
    String latitude;// = Double.parseDouble(MainActivity.lati); // 위도 36.5

    private AirWeatherObject originalAirWeatherObject;
    private String weather_type;
    private String str_Humidity, str_Temperature, str_Weather, str_Air;

    private String fullCityName;// = Stringaddr;
    //private String cityName = Stringaddr.split(", ")[2];
    double pm10Value;
        ...
        try {
            ...
            for (int i = 0; i < lists.size(); i++) {
                ...
                if(list.cityName.equals(guName)) {
                    //System.out.println(list.cityName + " / " + guName);
                    //System.out.println(list.sidoName + " / " + list.cityNameEng + " / " + list.dataTime + " / " + list.pm10Value);
                    ...
                    originalAirWeatherObject.humidity = (int) (originalAirWeatherObject.humidity + 0.5);
                    str_Humidity = Double.toString(originalAirWeatherObject.getHumidity());
                    originalAirWeatherObject.temperature = (int) (originalAirWeatherObject.temperature + 0.5);
            ...
            Type type = new Type(originalAirWeatherObject);
